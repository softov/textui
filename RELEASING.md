...
Seven, in dependency order - `scripts/release-publish.mjs` works this out itself:

```
@textui/core
  -> @textui/terminal -> @textui/widgets
       -> @textui/chat, @textui/cli, @textui/testing, @textui/kit
```
...
That bootstrap is done once per package; the first six in this repository went through it before 0.1.0, and `@textui/chat` before 0.6.0. If you add a package to the set later, it needs the same - and check the name is publishable *before* writing anything against it:
...
1. Publish it once with an npm **automation** token - not a classic publish token, which prompts for a one-time password and fails in CI with `EOTP`, and not a granular token restricted to specific packages, which cannot create a package that does not exist yet.
   Not straight from the workspace: the manifest there says `workspace:^`, and `npm publish` ships that string as it is, which is what `@textui/chat@0.6.0` did (it cannot be installed, and the version cannot be published again). `pnpm pack` in the package's directory writes the real ranges into the tarball; publish that tarball (`npm publish <name>-<version>.tgz --access public`).
...
2. Set the version on the seven publishable manifests, and move the `## Unreleased` heading in [`CHANGELOG.md`](CHANGELOG.md) down to the new version.
3. `pnpm check:version <version>` - it fails if any of the seven disagrees.
4. Tag and push:
