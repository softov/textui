...
app.screens.register({ id: 'detail', component: <Detail /> });
app.screens.reset('services');

app.screens.push('detail', { id: 'billing' });
app.screens.pop();          // focus is restored to where it was
```

A screen's own store scope (`$/screen.<id>/…`) is cleared when it is popped, unless it declared `keepAlive`.
