...
`draw` receives a [`PaintSurface`](https://github.com/softov/textui) clipped to the node's own rectangle - `put`, `text`, `fill`, `cell` and `clip` - and a render context carrying the resolved theme, the terminal's capabilities and the node's focus state.
