...
`pick` walks the **resource registry**, never the filesystem, so it works on whatever is mounted rather than on `file:` alone - the first thing anyone wants to pick off a remote is a file. `wants: 'directory'` adds a "Use this folder" row, because when the answer is the place you are standing there is no child to press enter on. Typing filters the list; `left` at the start of the filter goes up a level; enter on a folder goes into it.

## A focused field and the keys past its ends

A focused node sees a key before anything else, and a `TextInput` answers `left` and `right` itself. So a handler beside the field - even a global one - never sees those keys, and a feature built on one silently does nothing.
...
The command palette drills into a command's choices this way, and the path picker goes up a folder. `TextArea` has the same prop, for the same reason.
