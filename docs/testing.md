...
`render` mounts a node. `renderApp` takes a whole application - a shell, mounts,
registered commands - for the cases where the thing under test is the wiring
...
This is what `role` and `label` on a node are for. A component that sets
neither can only be found by its text, which is the assertion most likely to
change for reasons that are not bugs - see [Base props](components/base-props.md).
...
supposed to degrade; this is how you find out whether it does. See
