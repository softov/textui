...
A component names a role - `accent`, `danger`, `bulletFilled`, `single` - and the theme resolves it against what the terminal can actually do. That indirection is the whole design: it is why one component is right on all seven built-in themes, and on a terminal with sixteen colours and no Unicode.
