...
`diff` matters because owning the source means upstream fixes do not reach you.
It tells you which files have drifted, so an update is a decision rather than a
surprise.
...
unicode level, colour depth, mouse and keyboard protocol actually detected,
which is usually the answer.

## Building your own

`createCli()` is exported, and `@textui/cli/app` is the primitives under it -
commands, options, help and prompts. Deliberately small: the point is that an
application built on TextUI can combine plain commands with interactive screens
without a second framework and a second idea of what an argument is.
